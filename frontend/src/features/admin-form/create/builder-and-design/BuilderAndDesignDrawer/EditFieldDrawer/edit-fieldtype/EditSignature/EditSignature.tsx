...
type EditSignatureProps = EditFieldProps<NricSignatureBase>
...
export const EditSignature = ({ field }: EditSignatureProps): JSX.Element => {
  const {
    register,
    formState: { errors },
    buttonText,
    ...
    isLoading,
    handleCancel,
  } = useEditFieldForm<EditSignatureInputs, NricFieldBase>({
    field,
    transform: {
      input: (inputField) =>
  ...
  const requiredValidationRule = useMemo(
    () =>
      createBaseValidationRules<EditSignatureInputs, 'title'>({
        required: true,
      }),
    [],
  )
